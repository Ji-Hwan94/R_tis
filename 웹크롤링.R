...
html <- read_html(url, "UTF-8")
data_name <- html %>% 
  html_nodes(".tData01 tbody tr td:nth-child(2)") %>% 
  html_text()
data_avg <- html %>% 
  html_nodes(".tData01 tbody tr td:nth-child(4)") %>% 
  html_text()
...
data_loc <- html %>% 
  html_nodes(".table_develop3 tbody tr td:nth-child(1)") %>% 
  html_text()
data_deg <- html %>% 
  html_nodes(".table_develop3 tbody tr td:nth-child(6)") %>% 
  html_text()
data3 <- data.frame(loc=data_loc, deg=data_deg)
data3$deg <-as.numeric(as.character(data3$deg)) #as.numeric은 숫자로 변환 / as.character은 문자로 변환
data3 <- data3 %>% 
  arrange(desc(deg)) %>% 
  head(20)
...
html <- read_html(url, "UTF-8")
data_maker <- html %>% 
  html_nodes(".clearFix:nth-child(3) .left .title a") %>% 
  html_text()
library(stringr)
data_maker <- str_replace_all(data_maker, "\\W", "")
data_rate <- html %>% 
  html_nodes(".clearFix:nth-child(3) .left .share") %>% 
  html_text()
